...
export function SppPaymentForm({ bills, isSaving = false, onSubmit, students }: SppPaymentFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (Number.isNaN(amount) || amount < 0) {
      setValidationError("Nominal pembayaran tidak boleh negatif.");
      return;
    }
  ...
  return (
    <form className="grid gap-4 sm:grid-cols-2" onSubmit={handleSubmit}>
      ...
      <Input label="Nominal Pembayaran" type="number" value={formData.amount_paid} onChange={(event) => updateField("amount_paid", event.target.value)} required />
